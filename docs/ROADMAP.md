...
#### 4.1 Tailwind CSS 스타일 완성
- [ ] 전체 페이지 레이아웃 스타일 일관성 검토
  - 컨테이너 너비: `mx-auto max-w-screen-2xl px-4` 통일
  - 섹션 간격 일관성 확인
- [ ] 타이포그래피 스타일 적용
  - 제목 크기 계층 (h1 → h2 → h3)
  - 본문 가독성 최적화 (줄 높이, 자간)
- [ ] 카드 컴포넌트 hover 효과 추가
  - 이미지 scale, 그림자 전환
- [ ] 색상 시스템 검토
  - OKLCH 기반 CSS 변수 활용 (`globals.css`)
  - 라이트/다크 테마 색상 대비 확인

#### 4.2 shadcn/ui 컴포넌트 통합
- [ ] 필요한 shadcn/ui 컴포넌트 추가 설치
  - `Badge` (CategoryBadge용)
  - `Button` (페이지네이션, 네비게이션용)
  - `Separator` (구분선용, 필요 시)
- [ ] 기존 컴포넌트에 shadcn/ui 적용
  - `cn()` 유틸리티로 클래스 병합 패턴 통일
  - 스타일 오버라이드 필요 시 `className` prop 활용

#### 4.3 다크 모드 지원 완성
- [ ] `next-themes` ThemeProvider 레이아웃 래핑 확인 (`app/layout.tsx`)
- [ ] 테마 토글 버튼 Navbar에 배치 확인
- [ ] 모든 컴포넌트 다크 모드 색상 검토
  - 카드 배경, 텍스트 색상
  - 이미지 오버레이 색상
  - 배지 색상
...
- [ ] 데스크탑(1200px+) 레이아웃 검토
  - 3열 그리드
  - 최대 너비 제한 (`max-w-screen-2xl`)

#### 4.5 SEO 및 메타 태그 설정
- [ ] `generateMetadata()` 함수 작성
  - 홈 페이지: 블로그명, 기본 설명
  - 포스트 상세: 작품 제목, 설명, OG 이미지 (ImageUrl 활용)
  - 카테고리 페이지: "[카테고리] 작품 목록"
- [ ] Sitemap 생성 (`app/sitemap.ts`)
  - 홈, 전체 포스트, 전체 카테고리 URL 포함
...
- [ ] Next.js `Image` 컴포넌트 적용 확인
  ...
  - `priority` 속성 (히어로 이미지)
  - `placeholder="blur"` 설정 (가능한 경우)
- [ ] 불필요한 클라이언트 컴포넌트 제거
  - `"use client"` 사용 최소화
  - 서버 컴포넌트 우선 활용
- [ ] 번들 크기 확인 (`npm run build` 출력 검토)
...
- [ ] 카테고리 페이지 테스트
  - 카테고리별 필터링 정상
  - 존재하지 않는 카테고리 접근 처리
- [ ] 404 페이지 표시 확인 (없는 slug 접근 시)

#### 5.2 크로스 디바이스 호환성 확인
- [ ] 모바일 브라우저 테스트 (Chrome DevTools 또는 실제 기기)
  - 375px (iPhone SE 기준)
  - 터치 인터랙션 확인
- [ ] 태블릿 레이아웃 확인 (768px)
- [ ] 데스크탑 레이아웃 확인 (1440px)
...
- [ ] 자동 배포 설정 확인
  - `main` 브랜치 푸시 시 자동 배포
- [ ] 첫 배포 실행 및 빌드 로그 확인
- [ ] 배포 완료 URL 확인 및 접속 테스트

#### 5.4 프로덕션 환경 최종 검증
- [ ] 배포된 사이트 전체 페이지 접속 확인
- [ ] 실제 모바일 기기에서 접속 테스트
- [ ] ISR 동작 확인
  - Notion에 새 글 작성 → 1시간 내 사이트 반영 확인
- [ ] Open Graph 이미지 미리보기 확인 (SNS 공유 시)
...
| **Phase 3** | 핵심 기능 구현 | 3-4일 | 2026-05-14 | 2026-05-17 | 진행 중 (40%) |
| **Phase 4** | 스타일링 및 최적화 | 1-2일 | 2026-05-18 | 2026-05-19 | 대기 |
| **Phase 5** | 테스트 및 배포 | 1일 | 2026-05-20 | 2026-05-20 | 대기 |
| **완료** | **MVP 릴리즈** | **~8일** | - | **2026-05-20** | - |
...
**최종 수정**: 2026-05-14
**버전**: 2.2
**상태**: 실행 중 (Phase 3-2 완료 - Phase 3-3 준비)
